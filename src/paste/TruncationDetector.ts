...
export class TruncationDetector {
  ...
  private nudgedTopics = new Map<number, number>(); // topicId → timestamp
  private nudgeCooldownMs = 30 * 60 * 1000; // 30 minutes between nudges per topic
   ...
   */
  private checkNearLimit(text: string): TruncationResult {
   ...
   */
  private checkRapidMultiPart(topicId: number, userId: string, now: number): TruncationResult {
   ...
   */
  private checkStructuralIncompleteness(text: string): TruncationResult {
  ...
  private looksLikeCode(text: string): boolean {
    ...
    for (const line of lines.slice(0, 30)) {
      ...
      // Log-like timestamps
      if (/\d{2}:\d{2}:\d{2}/.test(line)) codeSignals++;
      // Stack trace patterns
      if (/^\s+at\s/.test(line) || /Error:|Exception:|Traceback/i.test(line)) codeSignals++;
    }

    return codeSignals > lines.length * 0.3;
  }

  private countChar(text: string, char: string): number {
    let count = 0;

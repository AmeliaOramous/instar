...
import { TreeTraversal, type TraversalDependencies } from './TreeTraversal.js';
...
export class SelfKnowledgeTree {
  private config: SelfKnowledgeTreeConfig | null = null;
  private triage: TreeTriage;
  private traversal: TreeTraversal;
  private synthesis: TreeSynthesis;
  private generator: TreeGenerator;
  private probeRegistry: ProbeRegistry;
  private options: SelfKnowledgeTreeOptions;

  // Grounding cache (Phase 2)
  private groundingCache = new Map<string, { result: GroundingResult; createdAt: number; agentMdMtime: number }>();
  private groundingInProgress = new Map<string, Promise<GroundingResult>>();
   ...
   */
  async search(query: string, options?: SearchOptions): Promise<SelfKnowledgeResult> {
    ...
    const { fragments, errors } = await this.traversal.gather(
      nodesToSearch,
      triageResult.scores,
    ...
    let synthesisResult = { synthesis: null as string | null, tokensUsed: 0 };
    if (!degraded && fragments.length > 0 && budgetUsed < budgetLimit) {
      synthesisResult = await this.synthesis.synthesize(
        query,
        fragments,
        config.agentName,
      );
   ...
   */
  async dryRun(query: string): Promise<SearchPlan> {
    const config = this.loadConfig();
   ...
   */
  async ground(topic: string, platform?: string): Promise<GroundingResult> {
  ...
  private async doGround(topic: string, platform: string | undefined): Promise<GroundingResult> {
    const start = Date.now();
  ...
  private groundingCacheKey(topic: string, platform?: string): string {
  ...
  private getAgentMdMtime(): number {
    try {
  ...
  addNode(layerId: string, node: SelfKnowledgeNode): void {
    const config = this.loadConfig();
    if (!config) throw new Error('Tree config not found — run generateTree() first');
  ...
  removeNode(nodeId: string): void {
    const config = this.loadConfig();
    if (!config) throw new Error('Tree config not found');
  ...
  private loadConfig(): SelfKnowledgeTreeConfig | null {
    if (this.config) return this.config;

    const filePath = path.join(this.options.stateDir, TREE_FILENAME);
    try {
      const content = fs.readFileSync(filePath, 'utf-8');
      this.config = JSON.parse(content) as SelfKnowledgeTreeConfig;
      return this.config;
    } catch {
      return null;
    }
  ...
  private emptyResult(query: string, startTime: number, error: string): SelfKnowledgeResult {
    return {
      query,
      degraded: true,
      fragments: [],
  ...
  private logTrace(entry: TreeTraceEntry): void {

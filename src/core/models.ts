 ...
 */
export function resolveModelId(tierOrId: string): string {
  const key = tierOrId.toLowerCase();

  // Direct tier name match
  if (key in ANTHROPIC_MODELS) {
    return ANTHROPIC_MODELS[key as ModelTierName];
  }
 ...
 */
export function resolveCliFlag(tierOrFlag: string): string {
  const key = tierOrFlag.toLowerCase();

  if (key in CLI_MODEL_FLAGS) {
    return CLI_MODEL_FLAGS[key as ModelTierName];
  }

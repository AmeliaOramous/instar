...
export async function reflectJob(slug: string, opts: ReflectJobOptions): Promise<void> {
  ...
  console.log(
    pc.green(`  ✓ ${stats.successCount} success`) +
...
export async function reflectAll(opts: ReflectAllOptions): Promise<void> {
  const config = await loadConfig(opts.dir);
  const journal = new ExecutionJournal(config.stateDir);
  const days = opts.days || 30;
  const agentId = opts.agent;
...
export async function analyzePatterns(slug: string | undefined, opts: AnalyzeOptions): Promise<void> {
  const config = await loadConfig(opts.dir);
  const journal = new ExecutionJournal(config.stateDir);
...
function printPatternReport(report: PatternReport, showProposals?: boolean): void {
  console.log();
...
function printPattern(pattern: DetectedPattern): void {
...
export async function consolidateReflection(opts: ConsolidateOptions): Promise<void> {
  const config = await loadConfig(opts.dir);
...
export async function runReflection(slug: string | undefined, opts: ReflectRunOptions): Promise<void> {
  ...
  const model = (opts.model as 'fast' | 'balanced' | 'capable') || 'capable';

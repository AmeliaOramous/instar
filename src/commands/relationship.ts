...
import type { RelationshipRecord, UserChannel } from '../core/types.js';

function getRelationshipsManager(): RelationshipManager | null {
...
export async function listRelationships(options: { sort?: string }): Promise<void> {
  const manager = getRelationshipsManager();
  if (!manager) return;

  const sortBy = (['significance', 'recent', 'name'].includes(options.sort || '')
    ? options.sort as 'significance' | 'recent' | 'name'
    : 'significance');
...
export async function importRelationships(options: { file: string; dryRun?: boolean }): Promise<void> {
  const manager = getRelationshipsManager();
  if (!manager) return;
  ...
  for (const rec of records) {
    ...
    const channels = (rec.channels || []) as UserChannel[];
    ...
    let existing: RelationshipRecord | null = null;
    ...
    if (existing) {
      ...
      // Set notes
      if (rec.notes) {
        manager.updateNotes(record.id, rec.notes as string);
      }
...
export async function exportRelationships(options: { file?: string; minSignificance?: string }): Promise<void> {
  const manager = getRelationshipsManager();
  if (!manager) return;
